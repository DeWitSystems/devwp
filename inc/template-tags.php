...
if ( ! function_exists( 'devwp_posted_on' ) ) :
	...
	function devwp_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'devwp' ),
...
if ( ! function_exists( 'devwp_posted_by' ) ) :
	...
	function devwp_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'devwp' ),
...
if ( ! function_exists( 'devwp_entry_footer' ) ) :
	...
	function devwp_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'devwp' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'devwp' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'devwp' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'devwp' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
